...
class SAC(RLAlgorithm, Serializable):
    ...
    def _init_actor_update(self):
        ...
        if self._reparameterize:
            policy_kl_loss = tf.reduce_mean(log_pi - log_target1/self.scale_entropy)
        else:
            policy_kl_loss = tf.reduce_mean(log_pi * tf.stop_gradient(log_pi - (log_target1 - self._vf_t)/self.scale_entropy - policy_prior_log_probs))
        ...
        self._vf_loss_t = 0.5 * tf.reduce_mean((
            self._vf_t - tf.stop_gradient(min_log_target - self.scale_entropy * log_pi + policy_prior_log_probs)
        )**2)
