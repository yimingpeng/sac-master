...
"""
CG: this version of the SAC algorithm changes the loss function for policy update, paving the way
for the development of GEAC.
"""

class SACV1(RLAlgorithm, Serializable):
    """
    ...
    def __init__(
        ...
        super(SACV1, self).__init__(**base_kwargs)
    ...
    def _init_actor_update(self):
        ...
        actions, log_pi = self._policy.actions_for(observations=self._observations_actor_ph, with_log_pis=True)
        ...
        if self._reparameterize:
            policy_loss = -tf.reduce_mean(-self.scale_entropy * log_pi + log_target1)
        else:
            policy_loss = -tf.reduce_mean(log_pi * tf.stop_gradient(-self.scale_entropy * log_pi + (log_target1 - self._vf_t)))
        ...
        self._vf_loss_t = 0.5 * tf.reduce_mean((
            self._vf_t - tf.stop_gradient(min_log_target - self.scale_entropy * log_pi)
        )**2)
    ...
    def _init_training(self, env, policy, pool):
        super(SACV1, self)._init_training(env, policy, pool)
    ...
    def _get_feed_dict(self, iteration, batch):
        ...
        feed_dict = {
            ...
            self._observations_actor_ph: self._duplicate_observations(batch['observations'], 4),
