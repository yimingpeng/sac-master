...
"""
CG: Tsallis entropy regularized AC.
"""

class TAC(RLAlgorithm, Serializable):
    """
    ...
    def __init__(
        ...
        super(TAC, self).__init__(**base_kwargs)
    ...
    def _init_actor_update(self):
        ...
        policy_loss = -tf.reduce_mean(log_pi *
                                      tf.stop_gradient(
                                          - (self.scale_entropy * self._tsallisQ /(self._tsallisQ - 1.)) * tf.exp( (self._tsallisQ - 1.)*log_pi )
                                          + (self.scale_entropy * self._tsallisQ /(self._tsallisQ - 1.))
        ...
        self._vf_loss_t = 0.5 * tf.reduce_mean((
            self._vf_t - tf.stop_gradient(
                min_log_target - self.scale_entropy / (self._tsallisQ - 1.) * (tf.exp( (self._tsallisQ - 1.)*log_pi ) - 1.)
            )
        )**2)
    ...
    def _init_training(self, env, policy, pool):
        super(TAC, self)._init_training(env, policy, pool)
