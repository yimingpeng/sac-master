...
class EAC(Serializable):
    ...
    def __init__(
        ...
        self._Q_ensemble = NNQFunction(env_spec=self._env.spec, hidden_layer_sizes=tuple([value_func_layer_size for _ in range(value_func_layers_number)]), name='ensqf')
        ...
        for act_next_ph in self._acts_next_phs:
            qt = self._Q_ensemble.get_output_for(self._observations_ens_next_ph, act_next_ph, reuse=True)
        ...
        self._q_train = self._Q_ensemble.get_output_for(self._observations_ens_ph, self._obv_act_ph, reuse=True)
        self._q_ens_loss = 0.5 * tf.reduce_mean((self._q_train -
                                                  tf.stop_gradient(
                                                      self._scale_reward * self._rewards_ph + (1 - self._terminals_ph) * self._discount * self._q_ens_next)
                                                  ) ** 2)
            ...
            for epoch in gt.timed_for(range(self._n_epochs + 1), save_itrs=True):
                ...
                for t in range(self._epoch_length):
                    ...
                    for i in range(self._n_train_repeat):
                        ...
                        feed_dict = {
                            self._observations_ens_ph: batch['observations'],
                            self._obv_act_ph: batch['actions'],
                            self._observations_ens_next_ph: batch['next_observations'],
                            self._rewards_ph: batch['rewards'],
                            self._terminals_ph: batch['terminals'],
                        }
                        for i,ains in enumerate(self._alg_instances):
                            with ains[0].policy.deterministic(self._eval_deterministic):
                                feed_dict[ self._acts_next_phs[i] ] = ains[0].policy.get_actions(batch['next_observations'])
    ...
    def get_action(self, observation):
        ...
        action_values = np.zeros(len(recommend_actions))
        for i, ract in enumerate(recommend_actions):
            action_values[i] = self._Q_ensemble.eval([observation], [ract[0]])[0]
        ...
        # Option 4: report the average over all recommended actions.
        # recommend_actions = np.array([x[0] for x in recommend_actions])
        # return np.mean(recommend_actions, axis = 0), {}
