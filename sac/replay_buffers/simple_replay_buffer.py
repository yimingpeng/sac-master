...
class SimpleReplayBuffer(ReplayBuffer, Serializable):
    ...
    def random_batch_with_index(self, batch_size, index):
        rstate = np.random.get_state()

        np.random.seed(index)
        tabu_indices = np.random.rand(self._size)
        tabu_indices = tabu_indices < 0.5
        tabu_indices = np.array([i for i, x in enumerate(tabu_indices) if x])

        np.random.set_state(rstate)

        indices = np.random.randint(0, len(tabu_indices), batch_size)
        return dict(
            observations=self._observations[tabu_indices[indices]],
            actions=self._actions[tabu_indices[indices]],
            rewards=self._rewards[tabu_indices[indices]],
            terminals=self._terminals[tabu_indices[indices]],
            next_observations=self._next_obs[tabu_indices[indices]],
        )
